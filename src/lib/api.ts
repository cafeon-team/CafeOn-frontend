...
export async function apiOwnerListSeats(): Promise<ApiSeat[] | null> {
  ...
  try {
    ...
    // ⚠️ 예전엔 실패 이유를 그냥 버리고 null만 돌려줘서, 화면엔 "못
    // 불러왔어요"라고만 뜨고 정작 왜 안 됐는지(타임아웃/서버 다운/인증 만료/
    // CORS 등)는 아무 데도 안 남았어요. 브라우저 콘솔(F12 → Console 탭)에
    // 실제 원인을 남겨서, 화면 문구만으론 구분 안 되는 문제를 바로 확인할
    // 수 있게 해요.
    console.error("[cafeON] 좌석 목록 조회 실패:", err);
    return null;
  }
}
